...
| Hybrid | Semi模式的预准备GM回合（含草稿、骰子、选项、修正说明） |
...
| 模式 | 说明 |
|------|------|
| **Semi**（默认） | LLM接收通过Gemini生成的预准备回合，仅对当前状态进行最小限度的改写。若存在则使用 `*_hybrid.json` 剧本包；若当前回合没有预制草稿，则回退到普通场景上下文。 |
| **Full** | LLM接收场景上下文（地点、NPC、道具、敌人、线索）并更自由地生成GM回应。 |
